...
import { controlButtonStyles } from "./Controls";
...
const Dropper = ({ onSelect, buttonStyle }: DropperProps) => {
  ...
  const getColorLegacy = (e: React.MouseEvent) => {
    ...
    const x1 = pageX * 2;
    const y1 = pageY * 2;
    let [r, g, b] = pickerCanvas?.getImageData(x1, y1, 1, 1).data as unknown as [number, number, number];
  ...
  const getEyeDrop = () => {
    ...
    if (!window.EyeDropper) {
      ...
      eyeDropper
        ...
        .then((result: { sRGBHex: tc.ColorInput; }) => {
  ...
  return (
    <div>
      <div
        style={{ ...buttonStyle }}
        className={controlButtonStyles(coverUp)}
        onClick={getEyeDrop}
      >
